...
Оптимизации должны представлять из себя платформозависимые ассемблерные вставки. Это сделает код менее читаемым, однако поможет использовать больше мощности процессора, доступной нам.
Оптимизации должны быть представлены в трёх видах:
- Ассемблерная вставка в языке C
- Функция полностью написанная на ассемблере
- Интринсики

Будем выбирать количество бакетов в хэш-таблице такое, чтобы число $LoadFactor$, то есть среднее количество элементов в каждом бакете, было равно 15. При нормальном использовании этой структуры данных надо использовать $LoadFactor = 1.5$, однако в учебных целях было выбрано значение в 10 раз больше, чтобы была возможность применить большее количество оптимизаций.
...
### 3.4 Документация
Документация для программиста доступна по [ссылке](https://artemneskorodov.github.io/hashtab/html/index.html).
...
Программу удалось ускорить в $4.14$ раз. Это достаточно большое значение, которое также можно увеличить, однако, как было видно, уже последняя оптимизация добавила много проблем которые будет необходимо учитывать при дальнейшей разработке программы.
