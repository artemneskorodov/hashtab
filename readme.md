...
Основная задача проекта заключается в оптимизации поиска по хэш-таблице. Эта структура хранения в данном примере используется для вычисления количества повторений слов в полном собрании сочинений Шекспира. Небольшая доработка позволит использовать этот проект как библиотеку для хранения произвольных данных.
Для оптимизации этой программы нужно использовать возможности ассемблера. Это сделает код менее читаемым, однако поможет полностью использовать доступные нам мощности процессора.
...
- Для графического представления профиля программы используется программа **Qcachegrind**
...
Для включения оптимизаций используется константа __*_OPTIMIZE_HASH*__ в файле ```source/hashtab.cpp``` и константы __*_OPTIMIZE_STRCMP*__ и __*_OPTIMIZE_SEARCH*__ в файле ```source/linked_list.cpp```. Значение этих констант станет понятно в описании оптимизаций проведённых с таблицей. Стоит отметить что включение оптимизаций с помощью __*_OPTIMIZE_HASH*__ и __*_OPTIMIZE_STRCMP*__ безопасно при соответствии правилам хранения базы данных описанных в соседнем разделе. Информацию об опасностях включения оптимизации с помощью флага __*_OPTIMIZE_SEARCH*__ можно найти в [этом разделе](#searchopt).

### 3.2 Формат базы данных

Данный раздел дополнялся по ходу оптимизации, поэтому правила описанные далее станут понятными по ходу прочтения.

Правила для хранения базы данных:
- Каждое слово хранится в блоке из 32 байт и не может превышать это количество символов.
...
```
Программа разобьёт текст из файла 'path_to_your_data' на наборы последовательно идущих букв. Результат будет совпадать с форматом базы данных. База данных будет записана в файл 'path_to_output'.
...
Также некоторые функции в программе вызываются из разных мест, например, подсчёт хэша. Для того чтобы было удобно оценивать наиболее долго выполняющуюся функцию в поиске необходимо сделать так, чтобы суммарное время выполнения поиска было много больше времени выполнения инициализации хэш-таблицы, тогда в таблице программы **Qcachegrind** при сортировке по колонке `Self` сверху будут отображаться функции, связанные с поиском. Для этого каждое слово теста будем запускать по несколько раз.
...
#### 5.1.1 Возможность оптимизации для сравнения строк
...
#### 5.1.2 Возможность оптимизации для подсчёта хэша

Процессоры X86-64 с поддержкой SSE4.2 имеют инструкции вычисляющие хэш ***crc32***. Вероятно это поможет оптимизировать программу. Выберем для нашей таблицы этот алгоритм подсчёта хэша. Валидность данного выбора подтверждена в разделе [**проверка валидности кэша**](#hashcheck).

### 5.2 Программа без оптимизаций
- - -
...
Как видно, дольше всего работает функция подсчёта хэша, однако если учесть время функции стандартной библиотеки C `strcmp(...)`, то больше всего времени занимает функция `cmp_key(...)`. Начнём с её оптимизации.

### 5.3 Оптимизация сравнения строк
- - -
...
### 5.4 Оптимизация подсчёта хэш функции
- - -
...
### 5.5 Оптимизация поиска элемента в списке
- - -
...
Программу удало ускорить в $k$ раз. Это достаточно большое значение, которое также можно увеличить, однако, как было видно, уже последняя оптимизация добавила много проблем которые будет необходимо учитывать при дальнейшей разработке программы.
...
## 7. Проверка валидности выбора хэш функции
...
Такое значение $LoadFactor$ не случайно: это часть задания. Можно было бы сделать его меньше, но тогда было бы больше трат памяти и не было бы смысла оптимизировать пробег по связному списку.
Разброс величины достаточно велик, однако мы используем эту функцию только для доступа к хэш таблице, соответственно её скорость нам будет очень важна, а такой алгоритм хэша позволяет сильно оптимизировать программу.
