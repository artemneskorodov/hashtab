...
ht_error_t hashtab_dump(hashtab_t *ctx, const char *label, const char *file, const char *func, int line) {
    fprintf(ctx->dump_file,
            "================================================================================\n"
            "This dump is called from line %d in file '%s'\n"
            "From function: '%s'\n"
            "Dump comment: %s\n"
            "--------------------------------------------------------------------------------\n"
            "Free: %p\n"
    ...
    for(size_t i = 0; i < BucketsNum; i++) {
        fprintf(ctx->dump_file, "\t[  %03lu]: %14p | %14p |", i, ctx->buckets[i].head->prev, ctx->buckets[i].head->next);
    ...
    for(size_t cont = 0; cont < ctx->used_containers; cont++) {
        ...
        for(size_t i = 0; i < ListContainerSize; i++) {
            fprintf(ctx->dump_file, "\t[%11p]: %14p | %14p | %s\n", &lists[i], lists[i].prev, lists[i].next, lists[i].data.key);
        }
    }
